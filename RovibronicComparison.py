...
def E_evr(ID):
    ...
    iA = countInputA(ID)
    dEA, IEA = getData(ID, iA)
#    print (len(dEA), len(IEA))
    plotCompare(dE, IE, dEA, IEA)
    return
# End main function
...
def countInputA(ID):
    x = open('{0}_gauss_296K_0.5.txt'.format(ID), 'r')    # Open file for reading input
    f = x.readline()
