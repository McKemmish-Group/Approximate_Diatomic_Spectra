# Test function for calculating rotational, vibrational and electronic spectra with input read and output write
# Date: 31/7/2018
...
def TransitionElec(ID, max_E, max_v, max_J, EN, kT, Threshold, Total_Electronic_Spin, Symbol, Reflection_Symmetry):
    ...
    for E_Lower in range(0,max_E):
        for E_Upper in range(E_Lower,max_E):
            ...
            # Initialise: TEST FC "on"
            FC = 1
            if (ElecFactor != 0 and FC != 0):
                for v_Lower in range(0,max_v):
                    for v_Upper in range(v_Lower,max_v):
                        for J in range(0,max_J):
                            IEcheck = FC*ElecFactor*(2*J+1)*math.exp(-EN[E_Lower*Tot_Jv+v_Lower*max_J+J]/kT)
